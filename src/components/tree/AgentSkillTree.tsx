...
export default function AgentSkillTree({ initialAllocated = ['birth'], shortId }: Props) {
  ...
  return <div className="grid min-h-screen grid-cols-1 bg-[#09070b] text-stone-100 lg:grid-cols-[1fr_380px]">
    <div className="relative overflow-hidden border-r border-amber-500/10 bg-[radial-gradient(circle_at_50%_40%,rgba(120,53,15,.22),transparent_35%),radial-gradient(circle_at_50%_60%,rgba(127,29,29,.16),transparent_50%),#07060a]">
      ...
      <div ref={viewport} className="h-full min-h-[720px] cursor-grab overflow-hidden" onMouseDown={(e)=>setDrag({x:e.clientX,y:e.clientY,pan})} onMouseMove={(e)=>{ if(drag) setPan({x:drag.pan.x+e.clientX-drag.x,y:drag.pan.y+e.clientY-drag.y}); }} onMouseUp={()=>setDrag(null)} onMouseLeave={()=>setDrag(null)}>
        <svg viewBox="0 0 1000 900" className="h-full w-full" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})`, transformOrigin:'center' }}>
          ...
          <circle cx="500" cy="450" r="390" fill="none" stroke="rgba(251,191,36,.08)"/><circle cx="500" cy="450" r="250" fill="none" stroke="rgba(251,191,36,.06)"/>
          {EDGES.map((e) => { const a=NODES[e.from], b=NODES[e.to]; const on = allocated.has(e.from) && allocated.has(e.to); return <line key={`${e.from}-${e.to}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={on?'rgba(251,191,36,.85)':'rgba(120,113,108,.28)'} strokeWidth={on?4:2}/>; })}
          {Object.values(NODES).map((node) => { const on = allocated.has(node.id), avail = remaining > 0 && canAllocate(node.id, allocated), syn = node.id.endsWith('_sig'); const r = node.type === 'keystone' ? 22 : node.type === 'notable' ? 17 : node.type === 'start' ? 24 : 12; return <g key={node.id} onClick={(e)=>{e.stopPropagation();toggle(node);}} onMouseEnter={()=>setHovered(node)} onMouseLeave={()=>setHovered(null)} className="cursor-pointer"><motion.circle cx={node.x} cy={node.y} r={r+8} fill={syn?'rgba(127,29,29,.22)':'rgba(0,0,0,.35)'} stroke={on?'#fbbf24':avail?'#a16207':'#57534e'} strokeWidth={on?3:1.5} filter={on ? (syn?'url(#redGlow)':'url(#goldGlow)') : undefined} initial={false} animate={{ scale: pulseId === node.id ? 1.28 : on ? 1.06 : 1 }}/><circle cx={node.x} cy={node.y} r={r} fill={on ? (syn ? '#7f1d1d' : '#b7791f') : '#15110d'} stroke={node.type==='keystone'?'#f5d080':'#78716c'} strokeWidth="1.5"/><text x={node.x} y={node.y+r+20} textAnchor="middle" className={cn('fill-stone-400 text-[12px]', on && 'fill-amber-100')} style={{fontFamily:'Cinzel'}}>{node.label}</text></g>; })}
        </svg>
